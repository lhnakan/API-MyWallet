...
async function findUserTransactions (id) {
    await database.query('SET LC_MONETARY TO "pt_BR.utf8"');
    try {
...
async function create (transaction) {
    ...
    const date = parseInt(selectDate[8] + selectDate[9], 10);
    const month = parseInt(selectDate[5] + selectDate[6], 10);
    const year = parseInt(`20${selectDate[2]}${selectDate[3]}`, 10);
